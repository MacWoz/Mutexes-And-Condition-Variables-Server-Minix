 ...
 *   tell_cv:		send a request to CV on behalf of a process
 */
...
int signal;
{
/* Send a request to CV, without blocking.
 */
  ...
  if (signal) {
           m_ptr->m_type = 5; /* interrupted by signal */
  }
  else {
          m_ptr->m_type = 6; /* exiting */
  }
  r = asynsend3(CV, m_ptr, AMF_NOREPLY);
  if (r != OK)
